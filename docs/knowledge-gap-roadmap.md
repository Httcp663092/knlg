...
| 集成运维 | 已加深 | WebAPI、集成、监控、重试、WMS 出入库对账、接口补偿、库存一致性和运维案例已有，PLM/税务/银企等专项场景可继续加 |
...
| 集成 | WebAPI、通用集成和 WMS 出入库对账已补，PLM/MES/税务平台专项还可继续拆 | 后续补 PLM-BOM 变更同步、接口批量补偿、税务平台发票同步 |
| 环境运维 | 基础运维已有，容量治理、归档、日志留存和审计追溯还能加厚 | 补数据清理归档、接口报文留存、发布审计和性能基线 |
| BOS/开发 | 插件治理已有，但源码级插件案例还不够多 | 按保存校验、操作服务、单据转换、列表插件补案例 |
| 财务扩展 | 财务基础很厚，下一步更适合补专项分析 | 补预算执行分析、税务申报勾稽、集团管理分析 |
...
| 集团管理分析 | 合并后管理报表、事业部利润、内部转移定价和经营指标追踪 |
...
| 缺口 | 应补内容 |
| --- | --- |
| PLM / WMS 深度集成 | WMS 出入库对账已补，PLM 后续应补主数据边界、BOM 变更同步、状态回写、错误重试和跨系统对账 |
...
如果继续沿着财务、供应链和集成深入，建议下一轮补：

1. `集团管理分析、事业部利润和内部转移定价专题`
2. `预算执行分析、占用释放和费用报销联动专题`
3. `PLM-BOM 变更同步、工程变更和生产版本追溯专题`
4. `SQL 记录卡样例库二期：把真实单号、截图和报表条件沉淀成 verified`

采购结算、收入确认、寄售/VMI、委外协同、客户退货质检、售后索赔备件、渠道售后、合并报表、税务申报勾稽、MES 车间数据、WMS 出入库对账、权限审计、真实环境收集入口、常用 SQL 查询包和真实环境 SQL 示例库补完后，财务、供应链、制造、实施、安全内控、集成和数据排查之间已经形成更完整的项目作战图。下一步可以继续补集团管理分析、预算执行分析、PLM 工程变更，并在拿到真实单号和截图后把 SQL 模板升级成 verified 记录。
