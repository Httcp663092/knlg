...
| 数据 SQL / BI | 已加深 | 有阅读方法、核心表字典、常用查询包、真实环境 SQL 示例库、对账/库存排查、BI 分析体系和真实环境内容收集入口，后续可继续把实际项目 SQL 和 BI 指标标记为 verified |
...
| 财务扩展 | 财务基础很厚，预算执行、税务申报、集团管理分析和经营月报样例已补 | 后续按真实经营分析月报、预算执行报表和费用分摊案例继续升级 |
| 数据 SQL / BI | 示例库和 BI 体系已有，但还缺真实客户验证样本 | 后续拿真实单号、截图、报表条件、BI 指标字典和看板截图后升级为 verified |
...
| BI 二期 | 已新增 [BI 分析体系详解](06_data-model-sql/bi-analytics-system-guide.md)，后续可按真实指标字典、驾驶舱截图、ChatBI 问答记录和标准报表核对结果继续升级 |
...
| BI 经营分析 | 已补 BI 体系页，后续补真实销售毛利、库存周转、采购价格、客户回款、制造效率看板样例 |
...
4. `真实经营分析月报二期：用实际报表模板、预算版本、事业部维度和异常清单升级样例`
5. `SQL 记录卡样例库二期：把真实单号、截图和报表条件沉淀成 verified`
6. `行业场景包：商贸、制造、项目型、连锁零售、医药批号、食品保质期`

采购结算、收入确认、寄售/VMI、委外协同、客户退货质检、售后索赔备件、渠道售后、合并报表、集团管理分析、经营分析月报、财务和供应链通俗基础、BI 分析体系、预算执行、税务申报勾稽、MES 车间数据、WMS 出入库对账、PLM-BOM 工程变更、权限审计、源码级插件结构案例、数据归档审计治理、真实环境收集入口、常用 SQL 查询包和真实环境 SQL 示例库补完后，财务、供应链、制造、实施、安全内控、开发、集成、运维、BI 和数据排查之间已经形成更完整的项目作战图。下一步更适合进入真实代码、真实清理规则、真实经营月报、真实 BI 看板，并在拿到真实单号和截图后把 SQL 模板升级成 verified 记录。
