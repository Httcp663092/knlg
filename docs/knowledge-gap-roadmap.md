...
| 供应链反向业务 | 已加深 | 已有销售退货、红冲、退款、折让、客户退货质检、RMA、返修、报废、售后索赔和备件专题 |
...
| 模块 | 当前短板 | 建议补法 |
| --- | --- | --- |
| 供应链 | 主流程已厚，但渠道售后、服务站库存、经销商代修还没有独立专题 | 补渠道售后、备件铺货、服务站库存、代修结算和服务绩效 |
| 实施交付 | 权限基础已有，审计证据包和高危权限追溯原来偏薄 | 已补权限审计专题，后续可按真实客户权限导出做案例 |
| 集成 | WebAPI 和通用集成已有，PLM/WMS/MES 专项对账还可继续拆 | 补 WMS 出入库对账、PLM-BOM 变更同步、接口批量补偿 |
...
| 缺口 | 应补内容 |
| --- | --- |
| 渠道售后和服务站库存 | 经销商代修、服务站备件、客户现场备件、费用结算和服务绩效 |
...
| 缺口 | 应补内容 |
| --- | --- |
| PLM / WMS 深度集成 | 主数据边界、单据接口、状态回写、错误重试、跨系统对账 |
...
如果继续沿着财务、供应链和制造深入，建议下一轮补：

1. `渠道售后、服务站库存与经销商代修专题`
2. `发票税务申报勾稽、红字和未开票收入专题`
3. `SQL 记录卡样例库二期：把真实单号、截图和报表条件沉淀成 verified`

采购结算、收入确认、寄售/VMI、委外协同、客户退货质检、售后索赔备件、合并报表、MES 车间数据、权限审计、真实环境收集入口、常用 SQL 查询包和真实环境 SQL 示例库补完后，财务、供应链、制造、实施、安全内控、集成和数据排查之间已经形成更完整的项目作战图。下一步可以补渠道售后、发票税务勾稽，并在拿到真实单号和截图后把 SQL 模板升级成 verified 记录。
