...
5. 测试一次采购价目表、销售价目表、信用控制、批号出库、盘点差异和暂估冲回。
6. 做一次采购结算：入库暂估、发票到票、暂估冲回、价差调整和供应商对账。
7. 模拟销售退货、红字应收、红字发票、客户退款和成本冲回，记录对库存、应收、税额和毛利的影响。
8. 模拟客户退货质检：退回待检、判定返修或报废、换货、红字应收、退款和客户对账。
9. 模拟一次售后备件出库、保外维修收费、客户赔偿和供应商索赔，检查收入、成本和往来。
10. 模拟渠道售后：服务站备件补货、经销商代修、旧件返还、服务费结算和保内保外判断。
11. 做一次寄售/VMI 练习，区分库存地点、所有权、消耗结算和应收应付时点。
12. 做一次存货月结：清负库存、核算出库成本、处理暂估价差、生成凭证并和总账对账。
13. 模拟负库存、无成本、暂估未冲回、红字出库，记录系统表现和处理方法。
